...
**Last reconciled with code: September 17, 2026.** This describes the current working tree. See [P1 delivery](P1_DELIVERY.md) for recorded verification and rollout status, and [PRD](PRD.md) for product targets that are not all implemented.
...
| Table | Purpose |
| --- | --- |
| `sessions` | Case identity, lifecycle status, phase/scene/chapter, evidence, interviewer, revision, ending path/step, retention timestamps |
...
sequenceDiagram
  ...
  API->>API: Select validated answer or safe authored fallback
...
Provider calls have 20-second timeouts and output limits. Roleplay generation failure marks the turn failed without a partial transcript. Answer-validator rejection or failure substitutes newly authored revelations or the safe deflection. Host-judge failure does not discard an otherwise valid answer. Microphone rotation occurs with the completed transaction, normally after three questions unless the case overrides the limit.
...
flowchart LR
  ...
  Check -->|Rejected or unavailable| Safe[Authored revelation or safe deflection]
  Safe --> Commit
```

Private author persona, true timelines, lies' underlying truth, `neverReveal` lists, and the solution object are not passed to roleplay. Those fields remain authoring references. `shortDescription`, `voice`, `knownFacts`, public alibi and exhibit prose must therefore be suitable for their intended visibility.
...
The phone-hack screen is a placeholder; soundtrack/TTS and final video playback are not integrated. See [README](../README.md) for setup and test commands. Apply the pending integrity migration before using this code against an existing database, preserve other worktrees' migrations, and create fresh lobbies: historical public IDs cannot establish host ownership safely. Hosted deployment and physical multi-device rehearsal remain unverified.
